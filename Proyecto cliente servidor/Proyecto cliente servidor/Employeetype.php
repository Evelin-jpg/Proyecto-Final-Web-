...
//------------------------------------------------------------------------------------------------------
$consulta="select * from employeetype where status = 1";
$datos = $conn->query($consulta);
echo '<table class="table">';
echo "<tr>";
echo "<th><p>Namee</th></p>";
echo "<th><p>BaseSalary</th></p>";
echo "<th><p>Hourss</th></p>";
while ($registro = $datos->fetch_assoc()){
//$registro = $datos->fetch_array(MYSQLI_ASSOC);
echo '
            <tr>           
                <td>'.$registro["Namee"].'</td>
                <td>'.$registro["BaseSalary"].'</td>
                <td>'.$registro["Hourss"].'</td>
                <td><a href="FormUpdEmployeetype.php?EmployeeTypeID='.$registro["EmployeeTypeID"].'"><i class="bi bi-pencil-square"></i>
                <td><a href="DelEmployeetype.php?EmployeeTypeID='.$registro["EmployeeTypeID"].'" onclick="return confirm(\'¿Estás seguro de que deseas eliminar este registro?\')">
                <i class="bi bi-trash-fill"></i>';
 echo "</tr>";
}
echo"</table>";

echo'<td><a href="ForminserEmployeetype.html"><i class="bi bi-plus-square"></i>';
